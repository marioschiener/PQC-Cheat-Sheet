...
\begin{minipage}[t]{0.4\textwidth} % left
    ...
    \begin{algorithmbox}{Symbols}
        ...
        Best to Worst: \textcolor{themegreen}{$\blacksquare$} $n\leq2$, \textcolor{themeyellow}{$\blacksquare$} $n\in\{3,4\}$, \textcolor{themeorange}{$\blacksquare$} $n\in\{5,6\}$, \textcolor{themered}{$\blacksquare$} $n\in\{7,8\}$, \textcolor{themered!65!black}{$\blacksquare$} $n\geq9$
        \begin{tabbing}
            \=\tripleicon{\montserratbold n}{\faMicrochip}{themegreylight!50}{0.6}{\faKey}
            \=$\left[5^n,5^{n+1}\right]$ CPU kilo cycles for key generation\\
            \=\tripleicon{\montserratbold n}{\faMicrochip}{themegreylight!50}{0.6}{\faPen}
            \=$\left[5^n,5^{n+1}\right]$ CPU kilo cycles for signing\\
            \=\tripleicon{\montserratbold n}{\faMicrochip}{themegreylight!50}{0.6}{\faQuestionCircle}
            \=$\left[5^n,5^{n+1}\right]$ CPU kilo cycles for signature verification\\
            \=\tripleicon{\montserratbold n}{\faMicrochip}{themegreylight!50}{0.6}{\faLock}
            \=$\left[5^n,5^{n+1}\right]$ CPU kilo cycles for encryption / key encapsulation\\
            \=\tripleicon{\montserratbold n}{\faMicrochip}{themegreylight!50}{0.6}{\faUnlock}
            \=$\left[5^n,5^{n+1}\right]$ CPU kilo cycles for decryption / key decapsulation\\
            \>\doubleicon{\montserratbold n}{\faPen}{themegreylight!50}{0.6}
            \>$\left[2^n,2^{n+1}\right]$ KB of signature size\\
            \>\doubleicon{\montserratbold n}{\faLock}{themegreylight!50}{0.6}
            \> $\left[2^n,2^{n+1}\right]$ KB of ciphertext size\\
            \>\doubleicon{\montserratbold n}{\faKey}{themegreylight!50}{0.6}
            \> $\left[2^{n-5},2^{n-4}\right]$ KB of signature algorithm public key size\\
            \>\doubleicon{\montserratbold n}{\faKey}{themegreylight!50}{0.6}
            \> $\left[2^n,2^{n+1}\right]$ KB of encryption algorithm public key size\\
        \end{tabbing}
        \vspace{-6.6mm}
    \end{algorithmbox}
\end{minipage}
...
\begin{minipage}[t]{0.58\textwidth} % right
    ...
    \begin{algorithmbox}{How To Interpret The Cheat Sheets}
        ...
        {\bfseries This approach prioritizes easy interpretation and comparability of metrics and in general quick informational gain over absolute precision of data -- remember this is a cheat sheet, not a standard! This document is not intended to replace the study of algorithm specifications. It just aims to point you in the right direction quickly.} If you are looking for the original and precise data, you can find it \iconhref[\faLink]{https://github.com/marioschiener/PQC-Cheat-Sheet/blob/main/Data/Data_Evaluation.ods}{here}.\\[\baselineskip]
